...
import InterpreterClient from 'dfx-type/on-chain-wasm-interpreter/on-chain-wasm-interpreter';
...
export async function makeSetup(identity: Identity): Promise<ISetup> {
    ...
    const {canisterId, idlFactory} = await import('dfx/on-chain-wasm-interpreter/on-chain-wasm-interpreter');

    const interpreterClient: InterpreterClient = Actor.createActor(idlFactory, {
        agent,
        canisterId
    });
